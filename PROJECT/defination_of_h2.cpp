...
void saveToFile(Tour* tours, int& tourCount, const string filename) {// Function to save tour data to a file
    ...
    for (int i = 0; i <= tourCount; ++i) {//saves data to file
        fout << tours[i].name << " " << tours[i].destination << " " << tours[i].airport << " " << tours[i].landmark << endl;
    }
...
void loadFromFile(Tour* tours, int& tourCount, const string filename) {
    ...
    while (tourCount < MAX_TOURS && fin >> tours[tourCount].name >> tours[tourCount].destination >> tours[tourCount].airport >> tours[tourCount].landmark) {
...
void add_tour(Tour* tours, int& tourCount, const string filename) {//function to add data
    if (tourCount < MAX_TOURS) {
        ...
        getline(cin, tours[tourCount].destination);
        cout << "\nDestination airport Name: ";
        getline(cin, tours[tourCount].airport);
        cout << "\nFavourate Landmark: ";
        getline(cin,tours[tourCount].landmark);
...
void view_tours(Tour* tours, int& tourCount) {//this function views tour data 
    cout << "NAMES\t\tDESTINATION\t\tAIRPORT\t\tLANDMARK\n" << endl;

    for (int i = 0; i < tourCount; ++i) {
        cout << tours[i].name << "\t\t" << tours[i].destination << "\t\t" << tours[i].airport << "\t\t" << tours[i].landmark << endl;
    }
}
...
void replace_tour(Tour* tours, int& tourCount, const string filename) {//this function repalce tour data
    ...
    if (foundIndex != -1) {
        ...
        cout << "Enter Airport\n";
        getline(cin, tours[foundIndex].airport);
        cout << "Enter Landmark\n";
        getline(cin,tours[foundIndex].landmark);
